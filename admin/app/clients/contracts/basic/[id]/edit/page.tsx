...
export default function EditBasicContractPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    const [formData, setFormData] = useState({
        contract_type: "BASIC" as "BASIC" | "NDA",
            ...
            <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link
                            href={`/clients/contracts?tab=${formData.contract_type.toLowerCase()}`}
                            className="p-2 hover:bg-slate-100 rounded-full transition-colors"
                        >
                            <ArrowLeft className="w-5 h-5 text-slate-500" />
                        </Link>
                        <div>
                            <h2 className="text-2xl font-bold tracking-tight">契約編集</h2>
